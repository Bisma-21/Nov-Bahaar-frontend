import {
  OuterContainer, Container, InnerContainer,
  ImageContainer, DescriptionContainer,
  Title, TitleContainer, Category,
  PriceContainer, PriceBody, PriceTag, Price,
  DiscountContainer, Discount,
  ProductDetailContainer, Head, Description,
  WishListContainer, SaveProductContainer, WishListText,
  AddToCartContainer, CartText, AddToCartDiv, AddedToCartDiv, LeftContainer,
  ImageBody, ImageDivision, ReviewContainer, ReviewBody, AllReviewContainer,
  MyReviewContainer, AboveDiv, UserName, DateTime, BelowDiv, Comment, Rating, Heading,
  HeadingDiv, InputDiv, Input, ButtonDiv, RatingDiv, MyReviewBody, NotComment
...
const ProductDetailComponent = (props) => {
  ...
  const whishlistProducts = whishlistSelector?.whishlistDetails?.map((e) => e?.pId?._id)
  ...
  useEffect(() => {
    if (router.isReady) {
      const fun = async () => {
        const response = await fetch(DB_URI + `product/` + productId, {
          method: "GET",
          headers: {
  ...
  useEffect(() => {
    const fun = async () => {
      const response = await fetch(DB_URI + `review/` + productId, {
        method: "GET",
        headers: {
          "Content-type": "application/json"
        }
  ...
  const addToCartHandler = async (id) => {
    try {
      ...
      const response = await fetch(DB_URI + "cart/to-add", {
        method: "POST",
        headers: {
          "Content-type": "application/json",
  ...
  const saveProductHandler = async (id) => {
    ...
    try {
      ...
      const response = await fetch(DB_URI + "whishlist/add-product", {
        method: "POST",
        headers: {
          "Content-type": "application/json",
          "x-access-token": getToken()
  ...
  const removeProductHandler = async (id) => {
    ...
    try {
      const response = await fetch(DB_URI + "whishlist/remove-product", {
        method: "POST",
        headers: {
          "Content-type": "application/json",
          "x-access-token": getToken()
  ...
  const submitMyReview = async () => {
    ...
    try {
      ...
      const response = await fetch(DB_URI + `review/create/` + productId, {
        method: "POST",
        headers: {
          "x-access-token": getToken(),
          "Content-type": "application/json"
    ...
    <>
      ...
      <OuterContainer>
        <Container>
            ...
            <ImageBody >
              {
                ...
                selector.length
                  ...
                  selector[0]?.images?.map((f) => {
                    return <ImageDivision key={f} onClick={selectImageHandler.bind(this, f)}>
                      <img src={f} alt="" />
                    </ImageDivision>
                  })
                  : null
          ...
          {
            selector.map((e) => {
              return <InnerContainer key={e._id}>
                <ImageContainer>
                  ...
                  <WishListContainer>
                    {
                      whishlistProducts?.includes(productId)
                        ?
                        <SaveProductContainer onClick={removeProductHandler.bind(this, e._id)}>
                          <>
                            <ButtonComponent className="whishlisted" title="Whishlisted" />
                            <AiTwotoneHeart className="filled-heart" disabled={true} />
                          </>
                        </SaveProductContainer>
                        :
                        <SaveProductContainer onClick={saveProductHandler.bind(this, e._id)}>
                          <>
                            <ButtonComponent className="whishlist" title="Whishlist" />
                    ...
                    <AddToCartContainer>
                      {
                        checkProduct()
                          ...
                          <AddedToCartDiv>
                            <>
                              <ButtonComponent title="In Cart" className="already-in-cart-btn" click={addToCartHandler.bind(this, e._id)} disabled={true} />
        ...
        <ReviewContainer>
          <ReviewBody>
            ...
            {review.length ? review?.map((e) => {
              ...
              const starsArray = Array.from({ length: e.rating }, (_, index) => (
                <AiOutlineStar key={index} />
              ));
              return <AllReviewContainer key={e._id}>
                <AboveDiv>
                  <UserName >
                    {e.userId?.name}
                  </UserName>
              ...
              <NotComment>
                <div> No Comments Yet,<br />You are first to comment</div>
              </NotComment>
            </AllReviewContainer>}

          </ReviewBody>

          {Object.keys(users).length ? <MyReviewContainer>
            <HeadingDiv>
            ...
            <MyReviewBody>
              ...
              <ButtonDiv><ButtonComponent title="Submit" className="btn" click={submitMyReview} /></ButtonDiv>
            </MyReviewBody>
